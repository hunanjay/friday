...
**Agent system** (`app/agents/`): `supervisor.py` hand-builds the graph with `langchain.agents.create_agent` — a parent agent delegates via `delegate_to_<name>` tools to five sub-agents: `mail_agent`, `contact_agent` (the Contact Relationship Brain — `create_contact`/`record_contact_fact`), `calendar_agent`, `memos_agent`, `github_agent` (daily work report / 日报). `mail_agent` and `memos_agent` also borrow the shared `search_contacts` tool to resolve a named recipient or route a memo to an existing contact, without owning contact data themselves. `routing.py`'s `decide_route` is the sole routing policy: a `/agent-name` slash command seeds the checkpoint with a synthetic `delegate_to_*` tool call so the graph's conditional entry point jumps straight to that sub-agent, bypassing the parent model; everything else enters through the parent model, which decides whether to delegate or answer directly. `build_supervisor` rebuilds the graph and every sub-agent per request, with tools closed over the caller's `user_id`. Conversation state persists in Postgres via `langgraph-checkpoint-postgres` (`checkpointer.py`); the parent model sees a trimmed 20k-token history (`_trim_history` `before_model` middleware), while each delegated sub-agent instead runs a single self-contained task string with pronouns/dates/people already resolved by the parent, not a slice of conversation history. Long-term memory is separate from checkpoint state: the `user_memory` table (`infrastructure/db/repositories/user_memory.py`) holds `profile`/`preference`/`topic` facts; `profile` and `preference` facts are auto-injected into every agent's and the parent's system prompt on every turn (`supervisor.format_memory_block`), while `topic` facts are only reachable through the `remember_user_fact`/`search_user_memory`/`forget_user_fact` tools every agent carries — recall of those is a model judgment call, not guaranteed.
